...
OUTPUT_PATH = REPO_ROOT / "reports" / "ablations" / "cnn_long_t_5groups_250hz_4s_overlap.json"
...
def load_group_samples() -> tuple[list[WindowSample], dict[int, dict[str, object]]]:
    ...
    for group_index, eeg_path in zip(LONG_T_GROUP_IDS, select_latest_five_waveforms()):
        ...
        for task_key, mi_label, start_s, end_s in FIXED_TASK_WINDOWS:
            ...
            for rank, (window, start_sample, end_sample) in enumerate(windows):
                samples.append(
                    WindowSample(
                        ...
                        data=window,
                        ptp=float(np.ptp(window, axis=1).max()),
...
def main() -> None:
    ...
    aggregate_predictions = {"cnn": {"y_true": [], "y_pred": []}}
    ...
    for held_out_group in LONG_T_GROUP_IDS:
        ...
        cnn_training, cnn_proba = train_temporal_model(train_x, train_y, val_x, val_y, seed=1200 + held_out_group)
        cnn_pred = np.argmax(cnn_proba, axis=1)

        aggregate_predictions["cnn"]["y_true"].extend(val_y.tolist())
        aggregate_predictions["cnn"]["y_pred"].extend(cnn_pred.tolist())
    ...
    y_true = np.asarray(aggregate_predictions["cnn"]["y_true"], dtype=np.int64)
    y_pred = np.asarray(aggregate_predictions["cnn"]["y_pred"], dtype=np.int64)
    fold_accs = [report["methods"]["cnn"]["metrics"]["accuracy"] for report in fold_reports]
    fold_bal_accs = [report["methods"]["cnn"]["metrics"]["balanced_accuracy"] for report in fold_reports]
    ...
    report = {
        "protocol": {
            ...
            "artifact_rule": "train-fit / val-apply, reject windows with ptp or rms above median + 3*MAD, interpreted as blink-like large fluctuations",
            "preprocessing": {
